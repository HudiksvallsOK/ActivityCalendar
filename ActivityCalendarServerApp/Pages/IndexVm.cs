...
namespace ActivityCalendarServerApp.Pages
{
	public class IndexVm : PageModel
	{
		...
		public async Task SetDefaults()
		{
			foreach (var activityType in this.ActivityTypes)
			{
				await this.LocalStorageService.SetItemAsync(activityType.ActivityType.ToString(), activityType.IsIncluded);
			}
			await this.LocalStorageService.SetItemAsync(nameof(this.DaysHorizon), this.DaysHorizon);
			await this.LocalStorageService.SetItemAsync(nameof(this.ActivityOpacityFactor), this.ActivityOpacityFactor);
			await this.LocalStorageService.SetItemAsync("ActivityTypesStored", true);
		}
